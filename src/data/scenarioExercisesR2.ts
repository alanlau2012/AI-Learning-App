  ...
  },
  {
    ...
    strategyControls: [
      ...
      { id: 'fieldGranularityMode', label: '字段粒度', options: [
        ...
        { id: 'referenceIdAndHash', label: '引用 id + 不可逆指纹', description: '用 docId、chunkId、toolCallId、policyDecisionId、templateVersion、带盐 hash/不可逆指纹和结构化错误码还原证据，不长期留原文。', routingRules: [], metricEffects: [{ metricId: 'sensitiveFieldExposureRisk', direction: 'down', magnitude: 'large', deltaMode: 'absolute', delta: 18, explanation: '原文留存减少，短文本也不依赖可反查 hash。' }, { metricId: 'diagnosticResolutionRate', direction: 'up', magnitude: 'medium', deltaMode: 'absolute', delta: 10, explanation: '能通过引用对象回放关键状态。' }] },
      ] },
    ...
    events: [
      ...
      { id: 'fields-too-thin', title: '字段过少无法还原上下文', symptom: 'Trace 只记录成功失败、token 和延迟，无法知道进入 prompt 的文档版本、片段排序、模板版本和权限过滤结果。', triggerStrategyOptionIds: ['modelCallOnly', 'feedbackOnly'], correctDiagnosis: '敏感数据最小化不等于删除所有可诊断字段；应保留引用 id、带盐 hash/不可逆指纹、版本号、错误码和策略决策，而不是长期保存原文。', investigationOrder: ['列出复盘错误答案所需的最小字段集合。', '区分必须原文、可摘要、可不可逆指纹、可引用 id 回放的字段。', '检查 docId、chunkId、templateVersion、routeReason、policyDecisionId 是否覆盖。'], missedRisks: ['字段过少会让事故复盘只能靠猜测。', '过度最小化会让 Eval 和发布回归无法闭环。'], relatedConceptIds: ['trace', 'permission-governance', 'prompt-context'], nextStepRecommendations: ['建立字段分级：原文禁止默认留存，引用 id、不可逆指纹、版本和错误码默认保留。', '对投诉、高风险和 eval case 保留受控可回放证据。'] },
      { id: 'raw-fields-privacy-risk', title: '字段过多造成敏感数据风险', symptom: '事故复盘拿到了完整 prompt 和工具参数，但安全团队发现其中包含个人信息、合同条款和内部审批意见。', triggerStrategyOptionIds: ['rawFullText'], correctDiagnosis: '可诊断性不能建立在长期全量原文留存上；应将敏感原文改为短期受控证据，常规 Trace 保留结构化摘要、引用 id、带盐 hash/不可逆指纹和版本。', investigationOrder: ['盘点 Trace 中哪些字段包含个人、合同、权限、财务或审批敏感信息。', '判断每个字段是否必须原文，还是可用摘要、不可逆指纹或引用 id 替代。', '检查留存周期、访问审批、脱敏策略和审计记录。'], missedRisks: ['全量原文会扩大数据泄露和合规风险。', '访问审批变慢会让事故复盘错过最佳窗口。'], relatedConceptIds: ['permission-governance', 'trace', 'observability'], nextStepRecommendations: ['将常规 Trace 切到脱敏摘要与引用 id。', '建立异常样本的短期受控取证流程和访问审计。'] },
    ],
    reviewRubric: {
      prompt: '请判断为什么 Trace 有很多数据但仍不可诊断，并说明你会如何按 span 覆盖、字段粒度、敏感数据最小化、反馈/Eval/发布关联和治理留存顺序改造。',
      requiredFindings: ['必须区分可观测设计和随手打日志，不能只建议多打日志。', '必须指出关键链路 span、父子关系、caseId 和版本字段是复盘基础。', '必须说明敏感数据最小化不是删除证据，而是用摘要、带盐 hash/不可逆指纹、引用 id、版本和受控回放替代长期原文。'],
      acceptableActions: ['补齐 gateway、retrieval、rerank、prompt assembly、model call、tool call、permission check、feedback 的关键路径 span。', '建立字段分级：原文、脱敏摘要、带盐 hash/不可逆指纹、reference id、版本号、错误码和策略决策。', '用 releaseVersion、templateVersion、evalCaseId 和 policyDecisionId 支撑回归判断。'],
