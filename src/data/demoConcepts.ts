...
export const demoConcepts: KnowledgePoint[] = [
  {
    ...
    "mentalModel": "可以把 Token 理解为模型阅读和书写时使用的“最小账本格子”。人看到的是一句话，模型看到的是一串格子编号；工程团队要管理的不是自然语言表面长度，而是这些格子在计算、缓存和计费系统里的流动。",
    "mechanism": [
      "输入文本先经过分词器，被切分为若干 Token；同一个汉字、英文词、符号组合在不同模型中可能切分方式不同。",
      "每个 Token 会被映射为词表中的整数编号，再转换为向量，作为模型计算的输入。",
    ...
    "diagnosticQuestion": {
      ...
      "options": [
        ...
        {
          "id": "d",
          "text": "是否需要增加摘要缓存，减少相同附件的重复计算"
        }
      ],
      "correctOptionIds": [
        "b"
      ],
      "explanation": "账单和 TTFT 同时上升，而输出长度变化不大，最可能的问题是输入 Token 增加，尤其是附件正文、邮件线程、系统模板和历史上下文膨胀。换便宜模型、限制输出或增加缓存可能有价值，但优先级应建立在输入/输出 Token 拆分之后。",
      "troubleshootingPath": [
  ...
  },
  {
    ...
    "mentalModel": "可以把注意力理解为一次会议中的临时发言权分配。不是每句话都同等重要，模型会根据当前生成目标给不同上下文片段分配权重；工程上要做的是控制会议材料质量，而不是把所有材料都堆进会议室。",
    "mechanism": [
    ...
    "diagnosticQuestion": {
      ...
      "explanation": "问题来自冲突上下文同时进入模型，且旧版文档文本更接近用户问法。应先治理进入上下文的材料质量和优先级；低温、引用格式检查或扩大窗口都可能有帮助，但不能替代版本过滤和权威排序。",
      "troubleshootingPath": [
        "查看命中片段的版本、生效日期和来源",
  ...
  },
  {
    ...
    "mentalModel": "可以把 Prefill 理解为模型正式回答前的“读卷阶段”。题目、材料、附录和历史记录越长，读卷越久；读完后形成的笔记就是后续生成时可复用的缓存。",
    "mechanism": [
    ...
    "diagnosticQuestion": {
      ...
      "options": [
        ...
        {
          "id": "b",
          "text": "扩容 Decode 服务实例，优先降低输出阶段排队"
        },
        {
          "id": "c",
      ...
      "explanation": "TTFT 上升而 TPOT 不变，且工具返回体积明显增加，说明瓶颈更可能在 Prefill。应先控制输入上下文；扩容 Decode、调整前端刷新或增加输出预算都没有对准首字前瓶颈。",
      "troubleshootingPath": [
  ...
  },
  {
    ...
    "diagnosticQuestion": {
      ...
      "options": [
        ...
        {
          "id": "c",
          "text": "增加推理副本数，但不区分短问答和长代码任务的调度队列"
        },
        {
          "id": "d",
      ...
      "explanation": "首字稳定、输入不大但长输出耗时高，说明主要压力在 Decode。输入压缩、扩容和解码优化都可能有价值，但优先策略应围绕输出长度、TPOT、批调度和质量验证设计。",
      "troubleshootingPath": [
        "按任务类型统计输出 Token 分布",
        "拆分 TTFT 与 TPOT 指标",
  ...
  },
  {
    ...
    "mentalModel": "可以把 TTFT 理解为餐厅从点单到上第一道菜的时间。第一道菜快，用户知道厨房开始工作；但慢的原因可能是排队、菜单转交、备菜、厨房调度或服务员路线，而不只是厨师炒菜慢。",
    "mechanism": [
    ...
    "diagnosticQuestion": {
      ...
      "explanation": "TTFT 受排队、模型前处理、Prefill 和缓存命中共同影响。TPOT 稳定说明 Decode 不是首要问题；只优化输出阶段不能解释首字前的等待增长。",
      "troubleshootingPath": [
        "拆分端到端 TTFT 时间段",
        "检查网关排队、限流和路由日志",
        "检查 RAG 检索耗时和上下文长度",
  ...
  },
  {
    ...
    "mentalModel": "可以把 KV Cache 理解为模型读过上下文后留下的“分层笔记”。在服务端保留并能命中这份笔记时，后续生成可以少重复读；如果请求被送到没有笔记的实例，就要重新读全文。",
    "mechanism": [
    ...
    "diagnosticQuestion": {
      ...
      "explanation": "TTFT 上升且 KV Cache 命中率下降，优先怀疑会话请求没有路由到持有上下文缓存的实例，或扩缩容导致缓存热度丢失。扩容、压缩输出或扩大窗口可能影响其他指标，但不是该组合信号的首要解释。",
      "troubleshootingPath": [
        "按会话检查连续请求是否落到同一实例",
        "查看扩缩容和负载均衡策略变更",
        "分析缓存淘汰率和长会话显存占用",
  ...
  },
  {
    ...
    "mentalModel": "可以把模型网关理解为企业 AI 的“机场塔台”。飞机不是直接各飞各的，塔台要确认身份、分配航线、控制流量、记录轨迹并在异常时改道；模型请求也需要这样的统一协调。",
    "mechanism": [
    ...
    "enterpriseCase": {
      "title": "多个业务绕过平台直连模型",
      "scenario": "某集团内部多个团队各自接入外部模型 API，费用、权限和审计口径不一致。",
      "problem": "安全团队无法追踪哪些系统发送了敏感数据，平台团队也无法统一限流和成本预算。",
      "analysis": "缺少模型网关导致调用路径分散，治理动作只能靠事后报表和人工约束，无法在请求进入模型前执行策略。",
      "solution": "建立统一模型网关，要求生产应用通过网关接入；网关提供应用凭证、模型路由、配额、审计日志和异常降级能力。",
      "takeaway": "模型网关是企业 AI 从试点走向规模化的治理入口。"
    },
    "pitfalls": [
      "把模型网关做成薄转发层，只代理 URL，不承担治理和观测责任。",
      "允许业务长期保留直连模型的旁路，导致成本和审计数据不完整。",
      "只做鉴权，不做 Token 计量、错误分类和模型版本记录。",
    ...
    "diagnosticQuestion": {
      ...
      "options": [
        ...
        {
          "id": "b",
          "text": "先要求各业务 SDK 统一埋点，但暂时保留直连调用路径"
        },
        {
          "id": "c",
        ...
        {
          "id": "d",
          "text": "只在业务侧增加敏感词过滤，不调整模型调用入口"
        }
      ],
      "correctOptionIds": [
        "a"
      ],
      "explanation": "审计与治理需要请求链路统一入口和可观测数据。SDK 埋点、供应商统一或业务侧过滤可以作为补充，但只要生产调用仍能绕过网关，策略执行和日志完整性就无法保证。",
      "troubleshootingPath": [
        "盘点所有模型调用入口和凭证",
        "阻断或迁移生产旁路调用",
  ...
  },
  {
    ...
    "mentalModel": "可以把多模型路由理解为企业的“任务派单系统”。简单工单不必派最资深专家，紧急工单要看响应时间，合规工单要派内部人员；模型选择也要按任务特征和约束派单。",
    "mechanism": [
      "网关接收请求后识别任务类型、输入长度、敏感等级、质量要求和时延预算。",
      "路由策略读取候选模型的能力评测、成本、当前负载、错误率、地域和合规属性。",
      "系统先判断候选模型是否满足该任务的质量门槛，再在满足门槛的模型中比较成本、时延和稳定性；当能力不足或异常时，再切换更强或外部模型补位。",
    ...
    "enterpriseCase": {
      ...
      "scenario": "某企业办公助手把摘要、翻译、制度问答和代码解释全部路由到同一个高能力模型。",
      "problem": "月度成本快速上升，但离线评测显示许多简单任务使用中等模型也能满足质量要求。",
      "analysis": "平台缺少任务分型和质量门槛，把模型选择当成一次性配置；没有基于评测和线上观测持续调整。",
      "solution": "建立任务分类、模型能力评测和成本基线；简单任务优先内部或低成本模型，复杂任务和失败重试再升级到高能力模型。",
      "takeaway": "多模型路由的价值来自质量、成本和稳定性的动态平衡。"
    },
    "pitfalls": [
    ...
    "diagnosticQuestion": {
      ...
      "options": [
        {
          "id": "a",
          "text": "先按固定比例把低风险流量切到便宜模型，同时观察投诉率"
        },
        {
          "id": "b",
      ...
      "explanation": "多模型路由必须基于任务、质量、成本和稳定性数据。固定比例灰度、低价优先或单模型收敛都可能是阶段性策略，但没有评测和观测就无法判断哪些任务可以安全降级。",
      "troubleshootingPath": [
        "按业务场景拆分任务类型",
        "建立每类任务的质量评测集和最低门槛",
        "采集模型版本、成本、错误和用户反馈",
  ...
  },
  {
    ...
    "mentalModel": "可以把上下文窗口理解为模型当前桌面上的资料夹。桌面再大也有边界，放进来的资料会影响判断；真正的工程能力不是把所有材料堆上桌，而是选择、压缩和排序最有用的材料。",
    "mechanism": [
    ...
    "diagnosticQuestion": {
      ...
      "explanation": "问题不是模型不知道规则，而是关键约束在长会话中被截断或稀释。扩大检索范围和摘要工具日志可能有帮助，但如果不保留硬约束的优先级，仍会重复丢失关键规则。",
      "troubleshootingPath": [
        "检查请求实际携带的上下文内容",
        "确认硬约束是否被截断或排在低优先级位置",
        "压缩工具输出并保留关键结论",
  ...
  },
  {
    ...
    "mentalModel": "可以把 Agent Loop 理解为一名工程师处理任务的工作节奏：先看现场，再形成下一步计划，执行一个动作，检查结果，然后决定继续、返工或请人确认。区别是企业需要把这个过程记录成可审计的机器流程。",
    "mechanism": [
    ...
    "enterpriseCase": {
      ...
      "scenario": "某企业试点运维 Agent，发现服务异常后连续重启同一服务三次，仍未解决问题。",
      "problem": "Agent 只有工具调用能力，没有明确校验标准、重试上限和升级人工的条件。",
      "analysis": "Loop 缺少 Check 和 Stop 设计，导致它把“执行过动作”误当成“问题在推进”。生产 Agent 必须能识别无效动作和风险边界。",
      "solution": "为每类动作定义成功判据、最大重试次数、回滚策略和人工审批点；所有观察、动作和结果写入 trace。",
      "takeaway": "Agent 的关键不是能调用工具，而是能在循环中被约束、验证和停止。"
    },
    "pitfalls": [
    ...
    "diagnosticQuestion": {
      ...
      "options": [
        ...
        {
          "id": "d",
          "text": "把测试失败日志完整塞入上下文，减少 Agent 漏看信息的概率"
        }
      ],
      "correctOptionIds": [
        "b"
      ],
      "explanation": "问题在于 Agent Loop 不可控、不可追踪，缺少动作后的校验和停止条件。更多工具、更详细计划或更多日志都可能改善局部能力，但如果没有边界、证据和停止机制，仍会扩大风险。",
      "troubleshootingPath": [
        "还原每轮观察、动作、结果和判断依据",
        "定义测试失败修复的成功判据",
        "设置最大修改范围和重试次数",
  ...
  },
  {
    ...
    "mentalModel": "可以把 Skill 理解为企业里的标准作业程序加工具包。新人照着 SOP 做事，既有步骤也有模板和检查项；Agent 使用 Skill 时，也是读取一组任务指令和资源来稳定完成某类工作。",
    "mechanism": [
    ...
    "enterpriseCase": {
      ...
      "scenario": "某研发平台发现 Agent 每次处理 CI 失败都要重新摸索日志入口、常见失败模式和修复验证方式。",
      "problem": "不同 Agent 会话产出不稳定，修复路径不可复用，人工评审重复指出相同问题。",
      "analysis": "团队把经验停留在个体提示词和聊天记录中，没有沉淀为结构化、可版本化的任务能力。",
      "solution": "创建 CI Fix Skill，明确触发条件、Owner、日志定位步骤、常见失败分类、禁止跳过测试规则、修复后验证命令和 PR 汇报模板。",
      "takeaway": "Skill 能把一次成功经验变成组织可复用的 Agent 能力。"
    },
    "pitfalls": [
    ...
    "diagnosticQuestion": {
      ...
      "options": [
        ...
        {
          "id": "c",
          "text": "把所有代码规范文档都注入上下文，尽量减少遗漏"
        },
        {
          "id": "d",
          "text": "为 Skill 增加更多可调用工具，但暂不定义每个工具的使用边界"
        }
      ],
      "correctOptionIds": [
        "b"
      ],
      "explanation": "Skill 不是普通提示词口号。更严格的提示、更多规范和更多工具可能有帮助，但稳定能力需要任务边界、步骤、检查清单、证据要求、工具边界和验收格式共同约束。",
      "troubleshootingPath": [
        "梳理代码审查的高频风险类型",
  ...
  },
  {
    ...
    "mentalModel": "可以把 Issue Fix Agent 理解为一名默认受限执行的工程师助手，它能阅读工单、查代码、跑测试和提交补丁，但需要清楚的问题描述、修改边界、验证标准和资深工程师评审。",
    "mechanism": [
    ...
    "diagnosticQuestion": {
      ...
      "explanation": "没有复现、根因和边界的大范围修改不适合自动合入。扩大搜索、补 PR 描述或补测试都有价值，但优先级应是规范化问题、复现并定位，再做最小修改和验证。",
      "troubleshootingPath": [
