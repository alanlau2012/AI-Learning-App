...
2. **5 类动画仍走 `GenericMechanismAnimation` 通用画布**（token-flow / attention-map / context-window / model-router / issue-fix-flow）：这是本轮「只做 2 个真实画布」的临时收口态（已不泄漏 key、fallback 可读），**不是动画质量目标**。新讲若复用这些类型，要清楚它当前是通用时间轴，不是机制专用画布。
