...
## 2. 当前 10 个动画组件与覆盖的知识点

当前实现 10 个组件，通过配置复用覆盖多个知识点（**不为每一讲写独立动画**）。下表与 [content-schema.md](content-schema.md) §4 的 `hasAnimation` 严格一致。
...
| SkillLifecycleAnimation | `skill-lifecycle` | skill |
| IssueFixFlowAnimation | `issue-fix-flow` | issue-fix-agent |
| ObservabilityTraceAnimation | `observability-trace` | trace, observability |
| TokenRoiFlowAnimation | `token-roi-flow` | token-roi |

> 覆盖 10 组件 / 10 类型 / 21 知识点，满足“≥4 类动画、≥6 个动画配置”的内容门槛。
> 其余高动画优先级概念（如 semantic-space→`semantic-space`、batch-scheduling→`batch-scheduler`、pd-separation→`pd-separation`、cache-system→`cache-layers`、RAG 场景→`rag-pipeline`）为后续扩展，按本枚举补充，不阻塞首版。
...
### 3.1 AnimationCanvas 组件契约（10 个动画组件统一实现）
